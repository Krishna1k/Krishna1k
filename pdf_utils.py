...
def to_markdown(content: str, title: str = "", subtitle: str = "") -> str:
    """Convert the same marked-up `content` into GitHub markdown.

    RED (@@) -> red circle bullet, GREEN ($$) -> green circle bullet,
    "# " -> "## ", "## " -> "### ", rules/pagebreaks -> horizontal rule.
    """
    out = []
    if title:
        out += ["# " + title, ""]
    if subtitle:
        out += ["_" + subtitle + "_", ""]
    out += ["> Legend: \U0001F534 = exam-important (must-do)  |  "
            "\U0001F7E2 = common in both CBSE & NIOS", ""]
    for raw in content.splitlines():
        s = raw.rstrip()
        st = s.strip()
        if st == "<<<PAGEBREAK>>>" or st.startswith("==========") or \
                st.startswith("----------"):
            out.append("\n---\n")
        elif s.startswith("## "):
            ...
            if body.startswith("@@"):
                out.append(indent + "- \U0001F534 " + body[2:].strip())
            elif body.startswith("$$"):
                out.append(indent + "- \U0001F7E2 " + body[2:].strip())
            elif st == "":
                out.append("")
            else:
                out.append(s)
